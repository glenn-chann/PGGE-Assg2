...
new Flock()
    ...
    float sepWeight)
    {
        ...
        {
            //store the autonomous i object in the boids list to prevent multiple look ups 
            Autonomous boid = boids[i];
            for (int j = 0; j < enemies.Count; ++j)
            {
                //store the autonomous i object in the enemies list to prevent multiple look ups 
                Autonomous enemy = enemies[j];
                //use sqrMagnitude because its computationally less expensive.
                float dist = (enemy.transform.position - boid.transform.position).sqrMagnitude;
                //sqaure sepDist  to compensate for the squared distance
                if (dist < sepDist * sepDist)
                {
                    Vector3 targetDirection = (
                    ...
                    boid.TargetSpeed += Mathf.Sqrt(dist) * sepWeight;
    ...
    IEnumerator Coroutine_SeparationWithEnemies()
    {
        while (true)
        {
            foreach (Flock flock in flocks)
            {
                ...
                //added yield to give control back to unity after every flock
                yield return null;
            }
            yield return null;
        }
        ...
        while (true)
        {
            foreach (Flock flock in flocks)
            {
                if (flock.useAvoidObstaclesRule)
                {
                    ...
                    for (int i = 0; i < autonomousList.Count; ++i)
                    {
                        //store the autonomous object to prevent multiple look ups in the list
                        Autonomous Auto = autonomousList[i];
                        for (int j = 0; j < mObstacles.Count; ++j)
                        {
                            //store the Obstacle object to prevent multiple look ups in the list
                            Obstacle Obs = mObstacles[j];
                            //use sqrMagnitude because its computationally less expensive.
                            float distSqr = (Obs.transform.position - Auto.transform.position).sqrMagnitude;
                            //sqaure avoidance radius to compensate for the squared distance
                            if (distSqr < Obs.AvoidanceRadius * Obs.AvoidanceRadius)
                            {
                                Vector3 targetDirection = (Auto.transform.position - Obs.transform.position).normalized;

                                Auto.TargetDirection += targetDirection * flock.weightAvoidObstacles;
                                Auto.TargetDirection.Normalize();
                            }
                        }
                    }
                }
